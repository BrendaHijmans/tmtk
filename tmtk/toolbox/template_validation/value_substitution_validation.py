...
class ValueSubstitutionValidator(Validator):
    ...
    def check_data_source(self):
        ...
        """
        files_no_ext = [".".join(file.split(".")[:-1]) for file in os.listdir(self.source_dir) if os.path.isfile(file)]

        for counter, data_source in enumerate(self.df['Sheet name/File name'],
                                              start=self.n_comment_lines + 1):
            if pd.isnull(data_source):
                self.is_valid = False
                self.can_continue = False
                self.logger.error(" No data source referenced at row {} in column 'Sheet name/File name'"
                                  .format(counter + 1))
            if not pd.isnull(data_source) and data_source.rsplit('.')[0] not in self.template.sheet_names and \
                    data_source.rsplit('.')[0] not in files_no_ext:
                self.is_valid = False
                self.can_continue = False
    ...
    # TODO: check whether column names are present in referenced sheet/file and in tree sheet with
    # TODO: respective sheet name/file name.
